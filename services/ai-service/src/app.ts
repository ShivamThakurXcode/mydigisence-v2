...
import { generateRoutes } from './generate/generate.routes.js'
...
async function bootstrap() {
  ...
  await app.register(rateLimit, { max: 30, timeWindow: '1 minute' }) // strict limit for AI calls
  ...
  app.get('/health', async () => ({ status: 'ok', service: 'ai-service', timestamp: new Date().toISOString() }))
  await app.register(generateRoutes)

  await app.listen({ port: config.port, host: '0.0.0.0' })
  log.info(`AI service running on port ${config.port}`)
}
