...
export default function ProfileBuilderPage({ params }: { params: { workspace: string } }) {
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-3">
          <a
            href={`/workspace/${params.workspace}`}
            target="_blank"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="space-y-2">
            {SECTION_REGISTRY.map((section, idx) => (
              <div
                key={section.type}
                className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                  section.enabled ? 'border-border bg-card' : 'border-dashed border-border/50 bg-muted/20 opacity-60'
                }`}
              >
                {/* Drag handle */}
                ...
                {/* Edit button */}
                {section.enabled && (
                  <button className="h-8 w-8 rounded-md border border-border hover:bg-muted flex items-center justify-center transition-colors shrink-0">
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-border bg-muted/20 overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              ...
              {SECTION_REGISTRY.filter((s) => s.enabled).map((s) => (
