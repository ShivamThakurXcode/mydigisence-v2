...
exports.metadata = { title: 'Settings' };
...
function SettingsPage({ params }) {
    return (<div className="space-y-8 max-w-3xl">
      ...
      <div className="flex gap-1 border-b border-border overflow-x-auto">
        {SETTING_SECTIONS.map((s, i) => (<button key={s.title} className={`px-4 py-2 text-sm font-medium border-b-2 whitespace-nowrap transition-colors ${i === 0
      ...
      <div className="bg-card rounded-xl border border-border p-6 shadow-sm space-y-5">
        ...
        {SETTING_SECTIONS[0]?.fields.map((field) => (<div key={field.label} className="space-y-2">
            <label className="text-sm font-medium">{field.label}</label>
            {field.type === 'textarea' ? (<textarea placeholder={field.placeholder} rows={3} className="w-full px-3 py-2 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring resize-none"/>) : field.prefix ? (<div className="flex items-center">
                ...
                <input type={field.type} placeholder={field.placeholder} className="flex-1 h-10 px-3 rounded-r-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"/>
              </div>) : (<input type={field.type} placeholder={field.placeholder} className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"/>)}
          </div>))}
      ...
      <div className="bg-card rounded-xl border border-border p-6 shadow-sm space-y-5">
        ...
        <div className="space-y-4">
          {TOGGLES.map((t) => (<div key={t.key} className="flex items-center justify-between">
      ...
      <div className="bg-card rounded-xl border border-border p-6 shadow-sm space-y-5">
        ...
        <div className="space-y-4">
          {PRIVACY_TOGGLES.map((t) => (<div key={t.key} className="flex items-center justify-between">
              <span className="text-sm">{t.label}</span>
              <button className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${t.on ? 'bg-primary' : 'bg-muted-foreground/30'}`}>
                <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white shadow-sm transition-transform ${t.on ? 'translate-x-4' : 'translate-x-0.5'}`}/>
              </button>
