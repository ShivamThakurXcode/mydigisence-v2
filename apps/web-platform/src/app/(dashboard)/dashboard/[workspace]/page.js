...
exports.metadata = { title: 'Overview' };
const STAT_CARDS = [
...
function DashboardOverviewPage({ params, }) {
    ...
    return (<div className="space-y-8">
      ...
      <div className="flex items-start justify-between">
        ...
        <link_1.default href={`/dashboard/${workspace}/profile-builder`} className="h-9 px-4 rounded-md bg-primary text-primary-foreground text-sm font-medium flex items-center hover:bg-primary/90 transition-colors">
          Edit Profile
        </link_1.default>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {STAT_CARDS.map((stat) => (<div key={stat.label} className="bg-card rounded-xl border border-border p-5 shadow-sm">
      ...
      <div className="bg-card rounded-xl border border-border p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {QUICK_ACTIONS.map((action) => (<link_1.default key={action.label} href={`/dashboard/${workspace}/${action.href}`} className="flex gap-3 p-4 rounded-lg border border-border hover:border-primary/50 hover:bg-muted/40 transition-all group">
