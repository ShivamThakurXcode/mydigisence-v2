...
export default function BookingsPage({ params }: { params: { workspace: string } }) {
  return (
    <div className="space-y-6 max-w-6xl">
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-3">
          <Link
            href={`/dashboard/${params.workspace}/settings`}
            className="h-9 px-4 rounded-md border border-border text-sm font-medium hover:bg-muted transition-colors flex items-center gap-2"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border p-5 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-sm">
            {Array.from({ length: 31 }, (_, i) => i + 1).map((d) => (
              <button
                key={d}
                className={`h-7 w-7 rounded-md text-xs hover:bg-muted transition-colors mx-auto ${
                  d === 15 ? 'bg-primary text-primary-foreground' : ''
                }`}
              >
                {d}
              </button>
        ...
        <div className="lg:col-span-2 bg-card rounded-xl border border-border shadow-sm overflow-hidden">
              ...
              {['All', 'Pending', 'Confirmed', 'Completed'].map((f) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${
                    f === 'All' ? 'bg-primary text-primary-foreground' : 'hover:bg-muted text-muted-foreground'
                  }`}
                >
                  {f}
                </button>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center px-6">
            ...
            <Link
              href={`/dashboard/${params.workspace}/marketplace`}
              className="mt-4 h-9 px-4 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors inline-flex items-center"
            >
