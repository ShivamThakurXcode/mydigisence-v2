...
export default function OnboardingProfessionalPage() {
  ...
  const [form, setForm] = useState({ displayName: '', title: '', slug: '', bio: '', hourlyRate: '' })
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (data.success && data.data) router.push(`/dashboard/${data.data.slug}`)
      else setStep(3)
    } catch { setStep(3) }
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            {STEPS.map((s, i) => (
              <div key={s} className={`flex items-center gap-2 ${i === step ? 'text-foreground font-medium' : i < step ? 'text-primary' : 'text-muted-foreground'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {PROFESSIONAL_TYPES.map((pt) => (
                <button
                  key={pt.value}
                  onClick={() => setProfType(pt.value)}
                  className={`p-4 rounded-xl border text-left transition-all hover:border-primary/50 ${profType === pt.value ? 'border-primary bg-primary/5' : 'border-border bg-card'}`}
                >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <input type="text" placeholder="Jane Smith" value={form.displayName}
                  onChange={(e) => setForm({ ...form, displayName: e.target.value, slug: handleSlugify(e.target.value) })}
                  className="w-full h-10 px-3 rounded-md border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Professional title</label>
                <input type="text" placeholder="Senior Full-Stack Developer" value={form.title}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center">
                  ...
                  <input type="text" placeholder="jane-smith" value={form.slug}
              ...
              <div className="space-y-2">
                ...
                <textarea placeholder="Tell clients about yourself..." rows={3} value={form.bio}
              ...
              <div className="space-y-2">
                ...
                <input type="number" placeholder="75" value={form.hourlyRate}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => setStep(2)}
                disabled={!form.displayName || !form.slug}
                className="flex-1 h-11 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >Continue</button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <input
                  type="text" placeholder="e.g. React, Figma, Python..."
                  value={skillInput}
              ...
              {skills.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill) => (
                    <span key={skill} className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/10 text-primary text-sm">
                      {skill}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={loading}
                className="flex-1 h-11 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors"
              >{loading ? 'Creating profile...' : 'Create profile'}</button>
            </div>
          </div>
        )}
